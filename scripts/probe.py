"""Call any soma MCP tool over HTTP with the bearer token — a quick test client.
...
Token is read from $SOMA_API_TOKEN or the .env file; URL from $SOMA_MCP_URL
...
def _token() -> str:
    tok = os.environ.get("SOMA_API_TOKEN")
    if tok:
        return tok
    ...
    raise SystemExit("No SOMA_API_TOKEN in environment or .env")


async def main() -> None:
    tool = sys.argv[1] if len(sys.argv) > 1 else None
    args = json.loads(sys.argv[2]) if len(sys.argv) > 2 else {}
    url = os.environ.get("SOMA_MCP_URL", "http://127.0.0.1:8000/mcp")
